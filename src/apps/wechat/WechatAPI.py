...
from apps.wechat import wechatConfig
...
class WechatLogin(WechatAPI):
    def get_code_url(self):
        ...
        url = self.config.defaults.get('wechat_browser_code') + (
            ...
            % (self.config.APPID, parse.quote(self.config.REDIRECT_URI),
               self.config.SCOPE, self.config.STATE
               if self.config.STATE else ''))
        return url
    ...
    def get_code_url_pc(self):
        ...
        url = self.config.defaults.get('pc_QR_code') + (
            '?appid=%s&redirect_uri=%s&response_type=code&scope=%s&state=%s#wechat_redirect'
            % (self.config.APPID, parse.quote(self.config.REDIRECT_URI),
               self.config.PC_LOGIN_SCOPE, self.config.STATE
               if self.config.STATE else ''))
        return url
    ...
    def get_access_token(self, code):
        ...
        token, err = self.process_response_login(
            requests.get(
                self.config.defaults.get('wechat_browser_access_token'),
                params=params))
        if not err:
    ...
    def get_user_info(self, access_token, openid):
        ...
        return self.process_response_login(
            requests.get(
                self.config.defaults.get('wechat_browser_user_info'),
                params=params))
